...
(function($){
  ...
  async function loadFilters(){
    try{
      ...
      const classes = await (await fetch(base + 'classes?per_page=200')).json();
      const instr = await (await fetch(base + 'instructors?per_page=200')).json();
      const locs = await (await fetch(base + 'locations?per_page=200')).json();
      ...
      if(Array.isArray(classes)) classes.forEach(c=>{ $('.cfp-filter-class, .cfp-form-class').append('<option value="'+c.id+'">'+(c.name||('#'+c.id))+'</option>'); });
      if(Array.isArray(instr)) instr.forEach(i=>{ $('.cfp-filter-instructor, .cfp-form-instructor').append('<option value="'+i.id+'">'+(i.name||('#'+i.id))+'</option>'); });
      if(Array.isArray(locs)) locs.forEach(l=>{ $('.cfp-filter-location, .cfp-form-location').append('<option value="'+l.id+'">'+(l.name||('#'+l.id))+'</option>'); });
      if (CFP_ADMIN.presetClassId) { $('.cfp-filter-class, .cfp-form-class').val(CFP_ADMIN.presetClassId); await fetchClassDetail(CFP_ADMIN.presetClassId); }
    }catch(e){}
  }
  ...
  async function loadMonth(current){
    ...
    const url=CFP_ADMIN.restUrl+'schedules?'+params.toString();
  ...
  $(async function(){ await loadFilters(); window.__cfpCurrent=new Date(); await loadMonth(window.__cfpCurrent); wire(); });
})(jQuery);
